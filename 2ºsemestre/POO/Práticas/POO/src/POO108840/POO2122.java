...
public class POO2122 {
	...
	private static void alinea1(PrintStream out) {
		...
		e1.addActivity(new Sport(Modality.KAYAK, 10))
			.addActivity(new Catering(Option.FULL_MENU, 12))
			.addActivity(new Catering(Option.DRINKS_AND_SNACKS, 20));
		
		Event e2 = grandesEventos.addEvent(c2, LocalDate.parse("2022-06-30"));
		e2.addActivity(new Catering(Option.LIGHT_BITES, 20))
			.addActivity(new Culture(Option.ARCHITECTURAL_TOUR, 12))
			.addActivity(new Culture(Option.RIVER_TOUR, 12));

		Event e3 = grandesEventos.addEvent(c3, LocalDate.parse("2022-06-25"));
		e3.addActivity(new Catering(Option.LIGHT_BITES, 20))
			.addActivity(new Culture(Option.WINE_TASTING, 20));

		Event e4 = grandesEventos.addEvent(c1, LocalDate.parse("2022-07-10"));
		e4.addActivity(new Sport(Modality.BIKE, 16))
			.addActivity(new Culture(Option.WINE_TASTING, 20));
